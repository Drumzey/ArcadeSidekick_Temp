...
function DrawEnemiesUI(userArray) {
    ...
    var array = userArray.sort();
    for (var i = 0; i < array.length; i++) {
        enemies.push([array[i], '<li id="' + array[i] + '" class="ui-li ui-btn-up-c" style="white-space: normal;text-overflow: clip;font-size:70%;">' + array[i] + '</li>']);
    }

    var nameoutput = [];

    for (var j = 0; j < enemies.length; j++) {
        nameoutput.push(enemies[j][1]);
    }
    ...
    for (var friend in friendData) {
        ...
        if (friendData.hasOwnProperty(friend)) {
            ...
            for (var game in friendData[friend]) {
                ...
                if (friendsNewGames.hasOwnProperty(game) &&
