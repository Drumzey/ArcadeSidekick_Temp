...
function PopulateComparison()
{
    ...
    if (friendOneDropDown === "Me" || friendOneDropDown === clientUserName) {
        ...
        if (currentRecord.scores.length > 0) {
            friendOnegamecount++;
            friendOneScores = currentRecord.scores;
        }
        else
        {
    ...
    if (friendTwoDropDown === "Me" || friendTwoDropDown === clientUserName) {
        ...
        if (currentRecord.scores.length > 0) {
            friendTwogamecount++;
            friendTwoScores = currentRecord.scores;
        }
        else {
            document.getElementById('noComparrisonText').innerText = "You have not played any games.";
            Show('#NoComparrison');
        ...
        //Has this friend actually played any games?
        for (var fg in friendsGames[friendTwoDropDown]) {
            if (friendsGames[friendTwoDropDown].hasOwnProperty(fg)) {
                friendTwogamecount++;
                break;
            }
        }
    ...
    for (var i = 0; i < friendOneScores.length; i++)
    {
        id = currentRecord.scores[i].id;

        if (friendsGames[friendTwoDropDown].hasOwnProperty(id))
        {
            var myscore = currentRecord.scores[i].score;
            var opponentsScore = friendsGames[friendTwoDropDown][id];

            if (myscore !== "0" && opponentsScore !== "0") {

                var whoWins = 0;

                if (timed.indexOf(id) !== -1) {
                ...
                var gameName = id.replace(/_/g, ' ').toLowerCase();
                var control = '<li name="head2head' + id + '" class="ui-li ui-btn-up-c" style="font-size:70%;white-space: normal;text-overflow: clip;">' + gameName + '</li>';
                head2headgameul.push([id, control]);

                if (whoWins === 0) {
                    control = '<li name="head2head' + id + '" class="ui-li ui-btn-up-c bestScore" style="font-size:70%;white-space: normal;text-overflow: clip;">' + myscore + '</li>';
                    head2headmyscoreul.push([id, control]);

                    control = '<li name="head2head' + id + '" class="ui-li ui-btn-up-c" style="font-size:70%;white-space: normal;text-overflow: clip;">' + opponentsScore + '</li>';
                    head2headfriendscoreul.push([id, control]);
                }
                else
                {
                    control = '<li name="head2head' + id + '" class="ui-li ui-btn-up-c" style="font-size:70%;white-space: normal;text-overflow: clip;">' + myscore + '</li>';
                    head2headmyscoreul.push([id, control]);

                    control = '<li name="head2head' + id + '" class="ui-li ui-btn-up-c bestScore" style="font-size:70%;white-space: normal;text-overflow: clip;">' + opponentsScore + '</li>';
                    head2headfriendscoreul.push([id, control]);
                }
            }
        }
    ...
    for (var j = 0; j < head2headgameul.length; j++) {
        outputhead2headgameul.push(head2headgameul[j][1]);
        outputhead2headmyscoreul.push(head2headmyscoreul[j][1]);
        outputhead2headfriendscoreul.push(head2headfriendscoreul[j][1]);
    }
